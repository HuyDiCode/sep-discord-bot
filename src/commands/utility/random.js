...
const { createList, getList, pickFromList, deleteList, getAllLists } = require('../../utils/randomManager');
        ...
        .addSubcommand(subcommand =>
            subcommand
                ...
                .addStringOption(option => 
                    option.setName('items').setDescription('Items separated by commas').setRequired(true)))
            ...
            group.setName('list')
                ...
                .addSubcommand(sub =>
                    sub.setName('create')
                        ...
                        .addStringOption(opt => opt.setName('items').setDescription('Items separated by commas').setRequired(true)))
                .addSubcommand(sub =>
                    sub.setName('pick')
                ...
                .addSubcommand(sub =>
                    sub.setName('view')
                        ...
                        .addStringOption(opt => opt.setName('name').setDescription('List name (optional)').setRequired(false)))
                .addSubcommand(sub =>
                    sub.setName('delete')
    ...
    async execute(interaction) {
        ...
        if (group === 'list') {
            ...
            if (subcommand === 'create') {
                ...
                if (items.length === 0) {
                    return interaction.reply({ content: 'List cannot be empty!', ephemeral: true });
                }

                const success = createList(name, items, interaction.user.id);
                if (success) {
                    return interaction.reply({ content: `✅ List **${name}** created with ${items.length} items.`, ephemeral: true });
                } else {
                    return interaction.reply({ content: `❌ List **${name}** already exists!`, ephemeral: true });
                }
            } 
                ...
                if (result.error === 'not_found') {
                    return interaction.reply({ content: `❌ List **${name}** not found!`, ephemeral: true });
                }
                if (result.error === 'empty') {
                    return interaction.reply({ content: `⚠️ List **${name}** is empty!`, ephemeral: true });
                }
                ...
                if (name) {
                    ...
                    const itemsStr = list.items.length > 0 ? list.items.join('\n• ') : '(Empty)';
                    const embed = new EmbedBuilder()
                        .setColor(0x2ECC71)
                        .setTitle(`📜 List: ${name}`)
                    ...
                    if (names.length === 0) {
                        return interaction.reply({ content: 'No lists found.', ephemeral: true });
                    }

                    const embed = new EmbedBuilder()
                        .setColor(0x2ECC71)
                        .setTitle('📜 All Random Lists')
                        .setDescription(names.map(n => `• **${n}** (${allLists[n].items.length} items)`).join('\n'));
                ...
                if (success) {
                    return interaction.reply({ content: `✅ List **${name}** deleted.`, ephemeral: true });
                } else {
                    return interaction.reply({ content: `❌ List **${name}** not found!`, ephemeral: true });
                }
            }
        ...
        if (subcommand === 'coin') {
            ...
            if (min > max) {
                return interaction.reply({ content: 'Min cannot be greater than Max!', ephemeral: true });
            }
            ...
            if (items.length === 0) {
                return interaction.reply({ content: 'Please provide a valid list of items!', ephemeral: true });
            }
